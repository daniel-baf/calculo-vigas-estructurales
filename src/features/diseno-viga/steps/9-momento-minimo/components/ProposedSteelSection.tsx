import { RebarRow } from "@/features/diseno-viga/components/flexion/RebarRow"
import type { useMomentoMinimo } from "../useMomentoMinimo"

type Props = Pick<
  ReturnType<typeof useMomentoMinimo>,
  | "qty1"
  | "setQty1"
  | "no1"
  | "setNo1"
...
export function ProposedSteelSection(props: Props) {
  const { qty1, setQty1, no1, setNo1, qty2, setQty2, no2, setNo2, alertas, errors } = props

  return (
    <div className="space-y-4">
      <RebarRow
        prefix="v1"
        label="Armado Principal"
        qty={qty1}
        no={no1}
        onQtyChange={setQty1}
        onNoChange={setNo1}
      ...
      <RebarRow
        prefix="v2"
        label="Varillas Adicionales (Opcional)"
        qty={qty2}
        no={no2}
        onQtyChange={setQty2}
        onNoChange={setNo2}
