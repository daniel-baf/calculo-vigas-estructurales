...
export function useParametrosBasicos(): ParametrosBasicosState {
  ...
  const errors = useMemo(
    () => validarParametrosBasicos({
      fc: Number(fc), bw: Number(bw),
      h: Number(h),  rec: Number(rec),
      L: Number(L),
    }),
    [fc, bw, h, rec, L]
  );
  ...
  const getData = () => ({
    fc: Number(fc), fy, lambda, beta, d,
    gradoAcero, tipoConcreto, portico,
    bw: Number(bw), h: Number(h), rec: Number(rec), L: Number(L),
  });

  // Helpers para aceptar string|number desde inputs HTML
  const numSetter = (setter: (v: number) => void) =>
    (v: number | string) => setter(v === '' ? 0 : Number(v));

  return {
    fc, setFc: numSetter(setFc),
    gradoAcero, setGradoAcero,
    tipoConcreto, setTipoConcreto,
    portico, setPortico,
    bw, setBw: numSetter(setBw),
    h, setH: numSetter(setH),
    rec, setRec: numSetter(setRec),
    L, setL,
    fy, lambda, beta, d,
    errors, isValid,
    getData,
