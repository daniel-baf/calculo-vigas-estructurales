...
export function calcularDisenoM1Pos(
  params: M1PosParams,
  input: M1PosInput,
  nosPermitidos: number[] = [3, 4, 5, 6, 7, 8]
): M1PosResult | null {
  ...
  const jAsumido = 0.9;
  const muEtabs = (asEtabsVal * phiFlexion * (fy || 4200) * jAsumido * (d || 1)) / 100;
  ...
  const no1 = parseInt(input.no1) || 0;
  const n2 = parseInt(input.n2) || 0;
  const no2 = parseInt(input.no2) || 0;
  ...
  const sMuSismico = portico === 'P.E'
    ? `M1(+) = 0.50 · φ Mn- = 0.50 · ${(Math.abs(phiMnNeg) || 0).toFixed(2)} = ${(muSismico || 0).toFixed(2)} kgf-m`
    : portico === 'P.I'
      ? `M1(+) = 1/3 · φ Mn- = ${(Math.abs(phiMnNeg) || 0).toFixed(2)} / 3 = ${(muSismico || 0).toFixed(2)} kgf-m`
      : `M1(+) = 0 kgf-m (Pórtico Ordinario)`;

  const sAsReq = `As_req = (Mu · 100) / (φ · fy · j · d) = (${(mu || 0).toFixed(2)} · 100) / (0.9 · ${fy || 0} · 0.9 · ${(d || 0).toFixed(2)}) = ${(asReq || 0).toFixed(2)} cm^2`;

  const sA = `a = (As_prop · fy) / (0.85 · f'c · bw) = (${(asProp || 0).toFixed(2)} · ${fy || 0}) / (0.85 · ${fc || 0} · ${(bw || 0).toFixed(2)}) = ${(a || 0).toFixed(2)} cm`;

  const sPhiMn = `φ Mn = (φ · As_prop · fy · (d - a/2)) / 100 = (0.9 · ${(asProp || 0).toFixed(2)} · ${fy || 0} · (${(d || 0).toFixed(2)} - ${(a || 0).toFixed(2)}/2)) / 100 = ${(phiMn || 0).toFixed(2)} kgf-m`;

  const sDC = `D/C = Mu / φ Mn = ${(mu || 0).toFixed(2)} / ${(phiMn || 0).toFixed(2)} = ${(dc || 0).toFixed(2)}`;

  const sSeccionControlada = `c = a / β 1 = ${a.toFixed(2)} / ${beta} = ${c.toFixed(2)} cm < 0.375 d = ${limiteC.toFixed(2)} cm → Ok`;
